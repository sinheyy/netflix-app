...
const TvInformation = ({ tv }) => {
    ...
    return (
        <div>
            ...
            <div className='tv-lastest-ep'>최신 에피소드</div>
            <div className='tv-still-container'>
                <div className='tv-still-img'>
                    <div className="tv-still" style={{ backgroundImage: `url(https://image.tmdb.org/t/p/original/${tv?.last_episode_to_air.still_path})` }}></div>
                </div>
                <div className='tv-still-info'>
                    <div className='tv-still-name'>{tv?.last_episode_to_air.episode_number}. "{tv?.last_episode_to_air.name}"</div>
                    <div style={{ marginBottom: 5 }}>{tv?.last_episode_to_air.overview}</div>
                    <div>{tv?.last_episode_to_air.runtime}분 | {tv?.last_episode_to_air.air_date}</div>
                    <div></div>
                </div>
            </div>
        </div>
